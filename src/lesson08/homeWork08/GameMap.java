...
public class GameMap extends JPanel {
    ...
    private static final int DOT_PC = 2;

    private static final int STATE_DRAW = 0;
    private static final int STATE_WIN_HUMAN = 1;
    private static final int STATE_WIN_PC = 2;
    ...
    private boolean turnAIWinCell() {
        for (int i = 0; i < fieldSizeY; i++) {
            for (int j = 0; j < fieldSizeX; j++) {
                if (isEmptyCell(j, i)) {
                    field[i][j] = DOT_PC;               // поставим нолик в каждую клетку поля по очереди
                    if (checkWin(DOT_PC)) {
            ...
            for (int j = 0; j < fieldSizeX; j++) {
                if (isEmptyCell(j, i)) {
                    ...
                    if (checkWin(DOT_HUMAN)) {            // если игрок победит
                        field[i][j] = DOT_PC;            // поставить на то место нолик
                        return true;
                    }
