...
public class GameMap extends JPanel {
    ...
    private void showMessageGameOver(Graphics g){
        ...
        switch(stateGameOver){
                ...
                case STATE_WIN_HUMAN:
                    g.drawString("Победил игрок", 100, getHeight() / 2 );
                    break;
                case STATE_WIN_PC:
    ...
    public void aiTurn() {
        ...
        if (turnHumanCell()) { // выиграет ли AI на следующем ходу
            return;
        }
        int x;
        int y;
        ...
        for (int i = 0; i < fieldSizeX; i++) {
            for (int j = 0; j < fieldSizeY; j++) {
                if (checkLine(i, j, 1, 0, winLength, c)) {
                    return true; // линия по Х
                }
                if (checkLine(i, j, 1, 1, winLength, c)) {
                    return true; // линия по диагонали Х Y
                }
                if (checkLine(i, j, 0, 1, winLength, c)) {
                    return true; // линия по Y
                }
                if (checkLine(i, j, 1, -1, winLength, c)) {
                    return true; // линия по диагонали Х -Y
                }
